import type { Translations } from './types';

export const ja: Translations = {
  languageName: '日本語',
  appTitle: 'Pomodoro',
  restMode: '短い休憩',
  longBreakMode: '長い休憩',
  settings: '設定',
  settingsSessionGroup: '1セットの設定',
  settingsLongBreakGroup: '長い休憩の設定',
  activeTimeLabel: 'ポモドーロ',
  restTimeLabel: '短い休憩',
  longBreakTimeLabel: '長い休憩',
  longBreakIntervalLabel: '長い休憩に入るまでのセット数',
  customMessageLabel: 'カスタムメッセージ',
  languageLabel: '言語',
  applySettings: '設定を適用',
  refreshData: 'データを更新',
  syncing: '同期中...',
  syncSuccess: '同期完了!',
  syncError: '同期に失敗しました',
  customInput: 'カスタム',
  presetSettingsLabel: 'プリセット設定',
  activePresetsLabel: 'アクティブ プリセット',
  restPresetsLabel: 'レスト プリセット',
  addPreset: '追加',
  restOffLabel: 'OFF',
  themeLabel: 'テーマ',
  themeLight: 'ライト',
  themeGray: 'グレー',
  themeDark: 'ダーク',
  alarmLabel: 'アラーム音',
  alarmSoundBell: 'ベル',
  alarmSoundDigital: 'デジタル',
  alarmSoundChime: 'チャイム',
  alarmSoundKitchen: 'キッチンタイマー',
  alarmSoundClassic: 'クラシック',
  alarmSoundGentle: 'ジェントル',
  alarmSoundSoft: 'ソフト',
  alarmSoundNone: 'なし',
  alarmRepeatLabel: 'アラーム音の繰り返し回数',
  alarmVolumeLabel: '音量',
  alarmVibrationLabel: 'バイブレーション',
  alarmVibrationOff: 'OFF',
  alarmVibrationSilent: '消音時のみ',
  alarmVibrationAlways: '常にON',
  alarmVibrationNote: '※ iPhoneでは非対応。Androidではアプリが画面に表示されている時のみ動作します',
  alarmChannelLabel: 'アラームの鳴らし方',
  alarmChannelMedia: 'ふつうに鳴らす',
  alarmChannelNotification: '画面オフでも鳴らす',
  alarmChannelNote: '※ 画面オフやマナーモードの時は「画面オフでも鳴らす」の方が鳴りやすくなります。通知の許可が必要です',
  alarmVolumeSystemControlNote: '※ 通知モードでは音量は端末の「通知音量」で調整します',
  alarmRepeatLockedNote: '※ 通知モードでは繰り返しは 1 回のみ',
  labelNote: 'タスクメモ',
  labelNotePlaceholder: 'このセッションのメモを入力...',
  labelSelectPlaceholder: 'ラベルを選択',
  addNewLabel: '+ 新しいラベル',
  labelStats: 'ラベル別集計',
  allLabels: 'すべて',
  labelsLabel: 'ラベル',
  addLabel: 'ラベルを追加',
  activeLabelLabel: '使用中のラベル',
  noLabel: 'ラベルなし',
  labelDuration: 'タイマー時間',
  labelDurationNone: 'なし（デフォルト）',
  weeklyStats: '統計・分析',
  weekly: '週間',
  monthly: '月間',
  yearly: '年間',
  today: '今日',
  thisWeek: '今週',
  totalTime: '合計時間',
  sessions: 'セット',
  previousWeek: '\u2190 前の週',
  nextWeek: '次の週 \u2192',
  previousMonth: '\u2190 前の月',
  nextMonth: '次の月 \u2192',
  previousYear: '\u2190 前の年',
  nextYear: '次の年 \u2192',
  exportCsv: 'CSVエクスポート',
  csvExportTitle: 'CSVエクスポート',
  csvExportDescription: 'セッションデータをCSVファイルとしてエクスポートします。',
  settingsTabGeneral: '全般',
  settingsTabLabels: 'ラベル',
  settingsTabPresets: 'プリセット',
  installApp: 'アプリとしてインストール',
  days: ['月', '火', '水', '木', '金', '土', '日'],
  months: ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'],
  defaultCustomMessage: '自分へのメッセージを入れてください',
  sessionChangeLabel: 'ラベルの変更',
  sessionEditNote: 'メモの修正',
  sessionDelete: '削除',
  sessionDeleteConfirm: 'このセッションを削除しますか？',
  confirmCompleteSession: 'セッションを完了しますか？',
  confirmComplete: '完了',
  confirmResetTimer: 'タイマーをリセットしますか？',
  confirmReset: 'リセット',
  csvImportTitle: 'CSVインポート',
  csvImportDescription: 'エクスポートしたCSVファイルからセッションデータを読み込みます。重複するセッションはスキップされます。',
  csvImportButton: 'CSVをインポート',
  csvImportModalDropzone: 'ここにCSVファイルをドロップ',
  csvImportModalOr: 'または',
  csvImportModalSelectFile: 'ファイルを選択',
  csvImportModalCancel: 'キャンセル',
  dataResetTitle: 'データの初期化',
  dataResetDescription: 'セッション履歴と設定をすべて削除します。この操作は取り消せません。',
  dataResetButton: 'データを初期化する',
  dataResetConfirm1: '本当に削除しますか？',
  dataResetConfirm2: '完全に削除する',
  dataResetCancel: 'キャンセル',
  authLogin: 'ログイン',
  authSignup: '新規登録',
  authLogout: 'ログアウト',
  authLoginWithGoogle: 'Googleでログイン',
  authOr: 'または',
  authEmail: 'メールアドレス',
  authPassword: 'パスワード（6文字以上）',
  authForgotPassword: 'パスワードをお忘れですか？',
  authCreateAccount: 'アカウントを作成',
  authProcessing: '処理中...',
  authErrorInvalidCredential: 'メールアドレスまたはパスワードが正しくありません',
  authErrorEmailInUse: 'このメールアドレスはすでに使用されています',
  authErrorWeakPassword: 'パスワードは6文字以上にしてください',
  authErrorInvalidEmail: 'メールアドレスの形式が正しくありません',
  authErrorLoginFailed: 'ログインに失敗しました',
  authErrorSignupFailed: 'アカウント作成に失敗しました',
  authVerificationSentTitle: '確認メールを送信しました',
  authVerificationSentMessage: '確認リンクをメールで送信しました。受信トレイを確認し、リンクをクリックして登録を完了してください。',
  authVerificationConfirm: 'OK',
  authUnverifiedTitle: 'メール未認証',
  authUnverifiedMessage: 'メールアドレスが認証されていません。受信トレイを確認し、認証リンクをクリックしてください。',
  authResendEmail: 'メールを再送信',
  authClose: '閉じる',
  authForgotPasswordTitle: 'パスワードのリセット',
  authForgotPasswordMessage: 'パスワードリセットリンクを受け取るメールアドレスを入力してください。',
  authSendResetEmail: 'リセットメールを送信',
  authResetEmailSent: 'パスワードリセットメールを送信しました。受信トレイをご確認ください。',
  authEmailVerifiedTitle: 'メール認証完了',
  authEmailVerifiedMessage: 'メールアドレスの認証が完了しました。すべての機能をご利用いただけます。',
  authOpenApp: 'アプリを開く',
  authTryAgain: 'もう一度試す',
  authNewPassword: '新しいパスワード（6文字以上）',
  authConfirmPassword: 'パスワードを再入力',
  authPasswordMismatch: 'パスワードが一致しません',
  authPasswordResetDoneTitle: 'パスワードリセット完了',
  authPasswordResetDoneMessage: 'パスワードがリセットされました。新しいパスワードでログインできます。',
  authDeleteAccount: 'アカウントを削除',
  authDeleteAccountConfirm: 'アカウントを削除しますか？すべてのデータが完全に失われます。',
  authDeleteAccountSuccess: 'アカウントが削除されました。',
  authChangePassword: 'パスワードを変更',
  authCurrentPassword: '現在のパスワード',
  authChangePasswordTitle: 'パスワードの変更',
  authPasswordChangeSuccess: 'パスワードを変更しました',
  authPasswordSameAsCurrent: '⚠ 新しいパスワードが現在のパスワードと同じです。このまま続けますか？',
  authUseSameAnyway: '同じパスワードで続ける',
  authSubmitChangePassword: 'パスワードを更新',
  authCancel: 'キャンセル',
  loadingMessage: '読み込み中...',
  labelChangeColor: '色を変更',
  labelRename: 'ラベル名を変更',
  labelDelete: '削除',
  customColorRegister: '登録',
  customColorDelete: '色を削除',
  colorPickerTitle: '色の選択',
  colorPickerConfirm: 'OK',
  colorPickerCancel: 'キャンセル',
  // Units
  minuteUnit: '分',
  sessionUnit: 'セット',
  labelNamePlaceholder: 'ラベル名を入力',
  statsSets: 'セット',
  statsTime: '時間',

  // Upgrade / Tier
  upgradeTitle: 'プランをアップグレード',
  upgradeDescription: 'この機能を使うにはStandard以上のプランが必要です。',
  upgradeStandard: 'Standard',
  upgradePro: 'Pro',
  upgradeStandardPrice: '¥250/月',
  upgradeProPrice: '¥5,000（買い切り）',
  upgradeRequired: 'アップグレードが必要です',
  freeLabelLimit: 'Freeプランではラベルは2個までです',
  planFree: 'Free',
  planStandard: 'Standard',
  planPro: 'Pro',
  upgradeCta: 'アップグレードする',

  // Standard → Pro upgrade
  upgradeToProTitle: 'Proにアップグレード',
  upgradeToProDescription: '買い切りプランでずっと使えます。月額の支払いが不要になります。',
  proDiscountInfo: 'Standard {months}ヶ月分（{amount}）を割引',
  proDiscountedPrice: '割引後: {price}',

  // Subscription management
  cancelSubscription: 'プランを解約',
  cancelSubscriptionDesc: 'Stripeの管理ページで解約手続きを行います',
  managePayment: 'お支払い方法の管理',
  cancelSubscriptionTitle: 'プランの解約',
  cancelSubscriptionWarning: '解約すると有料機能がすぐに利用できなくなります。この操作は取り消せません。',
  cancelSubscriptionTrialInfo: '{date}まで無料トライアル中',
  cancelSubscriptionDaysLeft: '残り{days}日',
  cancelSubscriptionConfirm: '解約する',
  cancelSubscriptionKeep: 'プランを維持',
  cancelSubscriptionSuccess: 'プランが解約されました',
  cancelSubscriptionError: '解約に失敗しました。もう一度お試しください。',

  // Free trial
  freeTrialBadge: '60日間無料',
  freeTrialNote: '無料トライアル後、自動的に月額プランに移行します',
  freeTrialMenuHint: 'Standardプラン60日間無料',

  // Payment / Checkout
  paymentSuccess: 'お支払いが完了しました！',
  paymentSuccessStandard: 'Standardプランへようこそ！すべての機能が解放されました。',
  paymentSuccessPro: 'Proプランへようこそ！すべての機能が解放されました。',
  paymentCancelled: 'お支払いがキャンセルされました。',
  checkoutLoginRequired: '購入にはログインが必要です。',
  checkoutProcessing: '決済ページへ移動中...',
  checkoutError: '決済処理に失敗しました。もう一度お試しください。',

  // Affiliate banner
  adDisclosure: '広告',
  affiliateCtaAmazon: 'Amazonで見る',
  affiliateUpsellTitle: 'PomoCare Pro',
  affiliateUpsellSubtitle: 'クラウド同期・エクスポートなど全機能を開放',
  affiliateUpsellCta: 'アップグレード',

  // Cookie consent banner
  cookieConsentMessage: 'サービス改善のため Google Analytics を使用します。拒否すると計測されません。',
  cookieConsentAccept: '同意する',
  cookieConsentDecline: '拒否する',

};
